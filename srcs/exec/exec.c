...
static int	ft_exec(t_cmd *cmd, t_list *env, int tmp[2], char **envp)
{
	...
	else
	{
		...
		else if (!g_pid)
		{
			...
			{
				if (execve(cmd->cmd[0], cmd->cmd, envp))
...
static int	exec_cmd(t_list **cmds, t_list *env, t_list *hist, char **envp)
{
	int		tmp[2];
	int		fd[2];
	int		fdpipe[2];
